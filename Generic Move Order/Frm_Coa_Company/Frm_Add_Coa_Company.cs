...
{
    public partial class Frm_Add_Coa_Company : Form
    {
        Connection connect = new Connection();
        bool status;

        Frm_Company frm;
        public Frm_Add_Coa_Company(Frm_Company _frm)
        {
            InitializeComponent();
            this.frm = _frm;
        }

        private void Frm_Add_Coa_Company_Load(object sender, EventArgs e)
        {
            AddOrEdit();
        }
        ...
        private void InsertCompany()
        {
            try
            {
                ...
                connect.con.Open();
                SqlCommand cmd = new SqlCommand("SP_InsertCoaCompany", connect.con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@code", text_code.Text);
                cmd.Parameters.AddWithValue("@company", text_company.Text);
        ...
        private void UpdateCompany()
        {
            try
            {
                connect.DatabaseConnection();
                connect.con.Open();
                SqlCommand cmd = new SqlCommand("SP_UpdateCoaCompany", connect.con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@id", edit_coa_company.id);
                cmd.Parameters.AddWithValue("@code", text_code.Text);
                cmd.Parameters.AddWithValue("@company", text_company.Text);
                cmd.Parameters.AddWithValue("@status", label_status.Text);
                cmd.Parameters.AddWithValue("@logged_user", User.id);
        ...
        {
            if (edit_coa_company.id > 0)
            {
                text_code.Text = edit_coa_company.code;
                text_company.Text = edit_coa_company.company;
                label_status.Text = edit_coa_company.status.ToString();
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            {
                if (text_code.Text == string.Empty || text_company.Text == string.Empty || cb_status.Text == string.Empty)
                {
